...
import { Assert, AssertionFactory } from '../src/assert.js';
import { MethodNames, Spy } from '../src/spy.js';
...
for (const useDefaultAssert of [true, false]) {
  await $it(`without mock implementation - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
  ...
  await $it(`with mock implementation - #1 - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
  ...
  await $it(`with mock implementation - #2 - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
    let counter = 1;
  ...
  await $it(`with call-through - all - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
  ...
  await $it(`with call-through - partial - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
  ...
  await $it(`call-through from mock implementation - with call-through - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
  ...
  await $it(`call-through from mock implementation - without call-through - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
    let fooCounter = 0, addCounter = 0;
  ...
  await $it(`call-through another method from mock implementation - with call-through - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
    let counter = 0;
    const spy: Spy<Dependency> = Spy.create(new Dependency(), true, {
      foo() {
        counter++;
  ...
  await $it(`call-through another method from mock implementation - without through - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
    let counter = 0;
    const spy: Spy<Dependency> = Spy.create(new Dependency(), false, {
      foo() {
        counter++;
  ...
  for (const callThrough of [true, false]) {
    await $it(`mocking single method - ${!callThrough ? 'without' : 'with'} call-through - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
    ...
    await $it(`mock member with a constant value - ${!callThrough ? 'without' : 'with'} call-through - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
    ...
    await $it(`mock property with a constant value - ${!callThrough ? 'without' : 'with'} call-through - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
  ...
  await $it(`captures methods arguments correctly - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
    const spy = Spy.create(new Dependency(), true);
    const sut = new Sut(spy.proxy);
  ...
  await $it(`captured calls can be cleared - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
    const spy = Spy.create(new Dependency(), true);
    const sut = new Sut(spy.proxy);
    assert.strictEqual(sut.add(1, 2), 3);
    assert.strictEqual(sut.add(new NumWrapper(1), new NumWrapper(2)), 3);
  ...
  await $it(`argument transformer can be used - ${useDefaultAssert ? 'default' : 'custom'} assertion`, function () {
    const spy = Spy.create(new Dependency(), true);
